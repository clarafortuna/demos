...
(function () {
  ...
  /** Radar chart: each axis is a section, polygon = DAC share profile. */
  function renderRadar(baseline, year, sections) {
    ...
    const axes = sections.map((s, i) => {
      ...
      return `
        ...
        <text x="${labelX.toFixed(1)}" y="${labelY.toFixed(1)}" text-anchor="${anchor}" dominant-baseline="middle"
              font-size="11" font-weight="600" fill="var(--text-2)">${escapeHtml(s.radarName)}</text>
      `;
    }).join('');
    const dots = ptsCurr.map((p, i) => {
      const s = sections[i];
      const v = s.pctByYear[year];
      const has = v != null;
      const pctText = has ? (v * 100).toFixed(1) + '%' : '—';
      const kpi = s.primaryKpi;
      const vv = kpi && kpi.values && kpi.values[year];
      const dacVal = vv ? vv.dac : null;
      const totalVal = vv ? vv.total : null;
      const kpiLabel = kpi ? kpi.label : '';
      const kpiUnit = kpi ? kpi.unit : '';
      return `<circle class="radar-dot" cx="${p[0].toFixed(1)}" cy="${p[1].toFixed(1)}" r="4" fill="var(--dusk)" stroke="white" stroke-width="1.5" data-section="${s.id}" data-name="${escapeHtml(s.name)}" data-pct="${pctText}" data-baseline="${baseline}%" data-kpi-label="${escapeHtml(kpiLabel).replace(/"/g, '&quot;')}" data-dac-val="${fmtKpiVal(dacVal, kpi)}" data-total-val="${fmtKpiVal(totalVal, kpi)}" data-unit="${escapeHtml(kpiUnit)}"/>`;
    }).join('');
    ...
    return `
      ...
      <div class="kpi-group">
        ...
        <div class="exec-shares-grid" id="exec-shares-grid">
          ...
          ${renderRadar(baseline, year, sections)}
        </div>
      </div>
    `;
  }
