 ...
 * suite_273 reads 6ee833b unless DAC_APP_OVERRIDE says
...
const NEW_COMMIT = process.env.DAC_NEW_COMMIT || '6ee833b';
const APP = path.join(os.tmpdir(), 'clcpa-273-app-' + NEW_COMMIT + '.js');
...
M.forEach((m) => {
  ...
  let out = '';
  try { out = execFileSync('node', ['suite_273.js'],
...
let clean = '';
try { clean = execFileSync('node', ['suite_273.js'],
    { cwd: DIR, encoding: 'utf8', env: Object.assign({}, process.env, { DAC_APP_OVERRIDE: APP }) }); }
