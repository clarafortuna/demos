 ...
 * suite_266 reads 2361a6a unless DAC_APP_OVERRIDE says
...
const APP = path.join(os.tmpdir(), 'clcpa-266-app-' + NEW_COMMIT + '.js');
...
M.forEach((m) => {
  ...
  let out = '';
  try { out = execFileSync('node', ['suite_266.js'],
...
let clean = '';
try { clean = execFileSync('node', ['suite_266.js'],
    { cwd: DIR, encoding: 'utf8', env: Object.assign({}, process.env, { DAC_APP_OVERRIDE: APP }) }); }
