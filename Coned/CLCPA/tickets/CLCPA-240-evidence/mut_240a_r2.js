...
const APP = path.join(os.tmpdir(), 'clcpa-240a_r2-app-' + NEW_COMMIT + '.js');
...
M.forEach((m) => {
  ...
  let out = '';
  try { out = execFileSync('node', ['suite_240a_r2.js'],
...
try { cleanOut = execFileSync('node', ['suite_240a_r2.js'],
    { cwd: DIR, encoding: 'utf8', env: Object.assign({}, process.env, { DAC_APP_OVERRIDE: APP }) }); }
