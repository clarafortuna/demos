 ...
 * suite_240b reads 2361a6a unless DAC_APP_OVERRIDE says
...
const APP = path.join(os.tmpdir(), 'clcpa-240b-app-' + NEW_COMMIT + '.js');
...
M.forEach((m) => {
  ...
  let out = '';
  try { out = execFileSync('node', ['suite_240b.js'],
...
let cleanOk = true;
try { execFileSync('node', ['suite_240b.js'],
    { cwd: DIR, encoding: 'utf8', env: Object.assign({}, process.env, { DAC_APP_OVERRIDE: APP }) }); }
catch (e) {
  cleanOk = false;
