...
/* Mutation controls for CLCPA-278-r4.
 *
 * The control that carries this round: the cell must follow the recompute,
 * and it must follow ONLY what the recompute wrote -- a repaint that
 * recomputes is the second source the bareNumber ruling forbade.
 *
...
const DIR = REPO + '/Coned/CLCPA/tickets/CLCPA-278-r4-evidence';
const SUITE = 'suite_278_r4.js';
const NEW_COMMIT = process.env.DAC_NEW_COMMIT || null;
const APP = path.join(os.tmpdir(), 'clcpa-278-r4-app.js');
...
const M = [
  { name: "THE DEFECT RETURNS: the return value is discarded again",
    from: "        const rewritten = recomputeDerivableSums(state.ingest.draft, state.ingest.schema,",
    to:   "        const rewritten = null; recomputeDerivableSums(state.ingest.draft, state.ingest.schema,",
    expect: "B2.user-added and so does the cell",
    alt: "B2.seed and so does the cell" },
  { name: "THE REPAINT recomputes instead of showing what was computed",
    from: "            ? formatIngestValue(state.ingest.draft[r][col], el.dataset.fmt === 'money')",
    to:   "            ? formatIngestValue(0, el.dataset.fmt === 'money')",
    expect: "B2.user-added and so does the cell" },
  { name: "THE REPAINT reaches the edited cell and fights it",
    from: "          if (col === c) return;",
    to:   "          if (false) return;",
    expect: "C5 and it does not fight the edited cell for its own value" },
  { name: "THE REPAINT ignores the formatter, so the cell disagrees with its column",
    from: "          el.value = el.dataset.fmt\n            ? formatIngestValue(state.ingest.draft[r][col], el.dataset.fmt === 'money')\n            : (state.ingest.draft[r][col] == null ? '' : String(state.ingest.draft[r][col]));",
    to:   "          el.value = state.ingest.draft[r][col] == null ? '' : String(state.ingest.draft[r][col]);",
    expect: "C4 through the same formatter the cell uses at rest" },
];

const lines = [];
const log = (...a) => { const s = a.join(' '); lines.push(s); console.log(s); };
log('======================================================================');
log('CLCPA-278-r4 -- mutation controls');
log('======================================================================');
...
fs.writeFileSync(DIR + '/mut-278-r4-output.txt', lines.join('\n') + '\n');
