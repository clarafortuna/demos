...
const OUT = path.join(__dirname, 'repro-293-r4-output.txt');

const lines = [];
const log = (...a) => { const s = a.join(' '); lines.push(s); console.log(s); };
const sleep = (ms) => new Promise(r => setTimeout(r, ms));

/* THE CHANGE IS IN app.js THIS ROUND, so BEFORE is the DEPLOYED build,
 * checked out into a temp directory, and AFTER is the working tree. The
 * deployed build is what is live in the org, which is the only comparison
 * that answers what this change does to the thing people are using. */
...
async function runOne(D, label, rows) {
  ...
  /* --- AND THEN SAVE IT, which is the half round 3 was missing -------
   *
   * The grid above is already past the editor's recompute, so it shows
   * whether the figure survived that. It does not show whether it survives
   * a SAVE, and the whole defect this round fixes is a figure that existed
   * for one instant and never reached the table. So: press Save Changes,
   * fill the dialog the app asks for, confirm, reload the page, and read
   * the row back out of storage. */
  await B.click('#ingest-save');
  await sleep(900);
  const dialog = await B.eval('!!document.getElementById("ingest-modal-confirm")');
  let savedRow = null;
  if (!dialog) {
    ...
    await sleep(1500);
    /* reload, so what is read back comes from storage and not from the
     * draft still sitting in memory */
    await B.goto(D.base + 'ExecutiveDashboard.html', '#view-container');
    await sleep(900);
    await D.gotoIngest();
    await D.pick('A', ID, YEAR);
    await sleep(800);
    const g2 = await B.eval('(function(){var o=[];' +
      'document.querySelectorAll("#ingest-editor-mount tr").forEach(function(tr){' +
      'var cs=[];tr.querySelectorAll("td,th").forEach(function(td){' +
      'cs.push(td.querySelector("input")?("["+td.querySelector("input").value+"]"):' +
      'td.textContent.trim());});o.push(cs);});return JSON.stringify(o);})()');
    let rows2 = [];
    try { rows2 = JSON.parse(g2 || '[]'); } catch (e) { rows2 = []; }
    savedRow = rows2.filter(r => /Total CES Programs/.test(String(r[0] || '')));
    if (savedRow.length) {
      savedRow.forEach(r => log('        AFTER SAVE AND RELOAD     : ' +
        JSON.stringify(r.slice(0, 4))));
    } else {
      log('        AFTER SAVE AND RELOAD     : (row not in the grid)');
    }
  }
  return { staged, stagedTxt, banner, grandRow, savedRow };
}
...
(async () => {
  ...
  log('before = the deployed build; after = this build, with the B7 registry');
  ...
  if (which === 'both' || which === 'base') await runBuild('BEFORE (the deployed build ' + BASE_REF + ')', baseDir());
  if (which === 'both' || which === 'fix') await runBuild('AFTER (this build)', DEV);
