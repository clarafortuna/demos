...
if (z) {
  ...
  const APPROVED = [
    ...
    '1. Go to the second sheet, named A.1 Incentive $. It shows the exact layout the import expects: the heading rows at the top, one row per program name, and (calculated) marking the cells the dashboard computes after import. Most tables have one heading row. A few have TWO, where a heading spans several columns and a second row names each one: keep both.',
    ...
    '3. Open the CSV you saved and fill in the values. Type values only in the positions the example shows empty; leave (calculated) and (no value) positions exactly as they are; do not change the heading rows or the program names. You MAY add new program rows at the bottom: the import will create them.',
    ...
    /* CLCPA-307 amends step 4, and the PIN is updated in full rather than
     * loosened, exactly as CLCPA-240 round 2 was. The step named two controls
     * that do not read that way on screen: CLCPA-234 renamed the page button
     * to "Add Data", and the button that commits a draft has always read
     * "Save Changes". CLCPA-226's rule is that a control has ONE name, and
     * generated text sending an operator to a button that is not there breaks
     * it in a file instead of in the page. suite_307 reads all four names out
     * of app.js by element id, so the next rename turns that suite red rather
     * than leaving this pin quietly describing a button nobody can find. */
    '4. In the dashboard, open Report Data and press Add Data, choose this section and table, then use Import From File and pick your CSV. The values land as a draft for your review; nothing is stored until you press Save Changes.',
