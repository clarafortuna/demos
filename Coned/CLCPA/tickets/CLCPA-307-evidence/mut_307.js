/* Mutation controls for suite_307.
 *
 * Control 3 is the point of the whole approach: it renames the BUTTON and
 * leaves the workbook alone, which is exactly how this defect was created in
 * the first place. A suite that hardcoded "Add Data" would stay green through
 * it and go stale the same way the instructions did.
 */
const fs = require('fs');
...
const SUITE = path.join(__dirname, 'suite_307.js');
const ORIGINAL = fs.readFileSync(APP, 'utf8');
const SHA = (s) => require('crypto').createHash('sha256').update(s).digest('hex').slice(0, 12);
const TMP = fs.mkdtempSync(path.join(os.tmpdir(), 'mut307-'));

const MUTATIONS = [
  {
    name: 'THE DEFECT ITSELF: step 4 goes back to the retired name',
    from: "        '4. In the dashboard, open Report Data and press Add Data, choose this ' +",
    to:   "        '4. In the dashboard, open Report Data, Add New Year, choose this ' +",
    expect: ['N2', 'N6'],
  },
  {
    name: 'step 4 shortens the commit button to "Save", which is not what it reads',
    from: "        'press Save Changes.' },",
    to:   "        'press Save.' },",
    expect: ['N4'],
  },
  {
    name: 'THE BUTTON is renamed and the workbook is left behind, which is how ' +
          'this defect was made',
    from: '<button class="btn btn-primary" id="ingest-addyear" type="button">Add Data</button>',
    to:   '<button class="btn btn-primary" id="ingest-addyear" type="button">Enter Data</button>',
    expect: ['N2'],
  },
];
...
console.log('  MUTATION CONTROLS: CLCPA-307');
