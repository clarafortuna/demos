...
(function () {
  ...
  /* CLCPA-215 FLIPPED THIS PIN, and the flip is the point of that ticket.
   *
   * It used to read "editing a WEIGHT alone leaves the stored mean, by design",
   * on the grounds that a 0.59% shift still rounds to the stored 0.45. Emely ruled
   * on 2026-09-03 that this was the gap wearing a nicer name: the operator changed
   * an input, so the output must move. The restoration is now keyed to whether the
   * INPUTS changed, not to whether the output rounds the same.
   *
   * The other half of the old pin is deliberately kept below, because it is the
   * half that was always right: idle E1 still shows and persists the source's
   * figure. Both halves in one place, so nobody re-derives the wrong one. */
  const dW = clone(o.base);
  dW[pi][1] = Number(dW[pi][1]) * 2;
  NEW.recomputeTotals(dW, o.sch, 'E1', o.base);
  check('E1: editing a WEIGHT now MOVES the mean (CLCPA-215)',
        dW[4][2] !== o.base[4][2], 'stayed at ' + dW[4][2]);
  check('E1: and to the weighted mean of the edited rows',
        Math.abs(dW[4][2] - 0.452639776127611) < 1e-12, String(dW[4][2]));
  check('E1: the weight edit itself is still preserved in the draft',
        dW[pi][1] === Number(o.base[pi][1]) * 2);
  // the half that survives: idle E1 keeps the source's figure
  check('E1: IDLE still shows the stored 0.45, not the full-precision mean',
        o.draft[4][2] === 0.45, String(o.draft[4][2]));
})();
