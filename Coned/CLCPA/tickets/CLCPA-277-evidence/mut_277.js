...
const M = [
  ...
  { t: SUITE, name: 'HARNESS: the baseline is repointed at a symbolic ref',
    from: "const BASE = process.env.DAC_BASE_COMMIT || 'cd7c7ed';",
...
M.forEach((m) => {
  ...
  let out = '';
  try { out = execFileSync('node', ['suite_277.js'], { cwd: DIR, encoding: 'utf8' }); }
...
let clean = '';
try { clean = execFileSync('node', ['suite_277.js'], { cwd: DIR, encoding: 'utf8' }); }
