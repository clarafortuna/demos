...
const NEW_COMMIT = process.env.DAC_NEW_COMMIT || 'ed44cae';
const APP = path.join(os.tmpdir(), 'clcpa-278-app-' + NEW_COMMIT + '.js');
...
const M = [
  ...
  { t: SUITE, name: 'HARNESS: the baseline is repointed at a symbolic ref',
    from: "const BASE = process.env.DAC_BASE_COMMIT || '4de6a8c';",
...
M.forEach((m) => {
  ...
  let out = '';
  try { out = execFileSync('node', ['suite_278.js'], { cwd: DIR, encoding: 'utf8', env: Object.assign({}, process.env, { DAC_APP_OVERRIDE: APP }) }); }
...
let clean = '';
try { clean = execFileSync('node', ['suite_278.js'], { cwd: DIR, encoding: 'utf8', env: Object.assign({}, process.env, { DAC_APP_OVERRIDE: APP }) }); }
