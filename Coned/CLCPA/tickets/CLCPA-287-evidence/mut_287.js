...
/* Mutation controls for suite_287. Temp copy via DAC_APP_OVERRIDE. */
const fs = require('fs');
const os = require('os');
...
const SUITE = path.join(__dirname, 'suite_287.js');
const ORIGINAL = fs.readFileSync(APP, 'utf8');
const TMP = path.join(os.tmpdir(), 'clcpa-287-app.js');
...
const MUTATIONS = [
  {
    label: 'the repaint is removed: the page goes silent again',
    from: "            refreshIngestNotices();",
    to:   "            /* removed */",
    expect: ['B1'],
  },
  {
    label: 'the whole view is rebuilt instead, which would destroy the dialog',
    from: "            refreshIngestNotices();",
    to:   "            rerenderIngestAll();",
    expect: ['B3'],
  },
  {
    label: 'the rejected plan is no longer stored, so there is nothing to draw',
    from: "              i.importResult = plan;",
    to:   "              i.importResult = plan.ok ? plan : null;",
    expect: ['D1'],
  },
  {
    label: 'clearIngestNotices stops emptying the field, breaking the 276 lifecycle',
...
log('======================================================================');
log('CLCPA-287 -- mutation controls');
...
if (base.failed !== 0) {
  ...
  fs.writeFileSync(__dirname + '/mut-287-output.txt', lines.join('\n') + '\n');
  process.exit(1);
}
