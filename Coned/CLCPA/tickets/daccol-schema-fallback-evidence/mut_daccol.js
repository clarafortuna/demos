...
const DIR = _dacRepo() + '/Coned/CLCPA/tickets/daccol-schema-fallback-evidence';
/* THE MUTATION TARGET IS THE PINNED BUILD, not the working tree.
 * suite_daccol reads 2361a6a unless DAC_APP_OVERRIDE says
...
const APP = path.join(os.tmpdir(), 'clcpa-daccol-app-' + NEW_COMMIT + '.js');
...
M.forEach((m) => {
  ...
  let out = '';
  try { out = execFileSync('node', ['suite_daccol.js'],
...
let cleanOk = true;
try { execFileSync('node', ['suite_daccol.js'],
    { cwd: DIR, encoding: 'utf8', env: Object.assign({}, process.env, { DAC_APP_OVERRIDE: APP }) }); }
catch (e) {
  cleanOk = false;
