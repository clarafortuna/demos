...
// (round-6 PREV control retired: PREV now points past round 6.)
...
lines.push('');
lines.push('=== 221: the four questions, in every entry ===');
{
  const st = after.api.st();
  const heads = ['What this data is', 'Where the data comes from', 'File requirements',
                 'How to update'];
  ['layers', 'indicators', 'shapes', 'coned', 'territory'].forEach(id => {
    after.api.dict(id, 'layers');
    const h = after.api.page();
    heads.forEach(q => ok(h.indexOf('<h3>' + q + '</h3>') >= 0, id + ': answers "' + q + '"'));
  });
  after.api.undict();
}

lines.push('');
lines.push('=== 221: the approved texts, verbatim ===');
{
  // Drift from the round 3 approved wording is a defect, so each is pinned by a
  // distinctive phrase rather than by a loose match.
  const pins = {
    layers: ['GeoJSON overlays you upload to draw over the map as toggleable layers',
             'Polygon or MultiPolygon features only', 'Up to 50 MB',
             'no crs member is accepted too'],
    indicators: ['NYSERDA per tract Disadvantaged Communities data',
                 'at least 98 percent of the tracts the map draws',
                 'opdgig.dos.ny.gov', 'convert_nyserda_raw.py'],
    shapes: ['census tract boundaries the map draws', 'Exactly 8 properties per tract',
             'census.gov', '8ius-dhrr', 'build_pure_geometry_dataset.py'],
    coned: ['Per tract account counts, EAP enrolment and adjustments',
            'keep their corrections', 'build_coned_dataset.py'],
    territory: ['ORU', 'No GEOIDs and no vintage', '_make_territories.py',
                'needs network access'],
  };
  Object.keys(pins).forEach(id => {
    after.api.dict(id, 'layers');
    const h = after.api.page();
    pins[id].forEach(frag => ok(h.indexOf(frag) >= 0,
      id + ': "' + frag.slice(0, 46) + '"'));
  });
  after.api.undict();
}
...
lines.push('');
lines.push('=== 221: the drawer is retired, nothing orphaned ===');
{
  const src = fs.readFileSync(path.join(REPO, REL), 'utf8');
  ...
  const h = after.api.page();
  ['Polygon or MultiPolygon', 'EPSG:4326', 'numeric on every feature'].forEach(f =>
    ok(h.indexOf(f) >= 0, 'absorbed from the drawer: "' + f + '"'));
  after.api.undict();
  ok(/>\s*Data Sources<\/button>/.test(afterByTab.layers.replace(/\n/g, ' ')),
     'the header button is renamed Data Sources');
  ok(/File Requirements/.test(prevByTab.layers), 'PREV control: it said File Requirements');
}
...
['indicators', 'shapes', 'coned', 'territory'].forEach(t => {
  const h = afterByTab[t];
  ok(/ds-session-card/.test(h), t + ': has a This Session card');
  ok(/<h3>This Session \(Unsaved\)<\/h3>/.test(h), t + ': titled like Tab 1');
});
ok(/version uploaded in this session stays here until you switch it on/.test(afterByTab.indicators),
   'indicators: names the uploaded-not-active state, which only it can have');
['shapes', 'coned', 'territory'].forEach(t => {
  ok(/publish on upload, so there is no separate step after that/.test(afterByTab[t]),
     t + ': says it publishes on upload rather than faking a waiting state');
...
{
  const src = fs.readFileSync(path.join(REPO, REL), 'utf8');
  ok(/let _dsRecordsLoaded = false;/.test(src), 'there is a records-ARRIVED flag');
  ok(/_dsRecordsLoaded = true;/.test(src), 'and it is set when they arrive');
  // it must be distinct from _dsHydrated, which is true during the whole window
  const hyd = src.indexOf('_dsHydrated = true;');
  const rec = src.indexOf('_dsRecordsLoaded = true;');
  ok(hyd >= 0 && rec >= 0 && hyd < rec,
     '_dsHydrated is set BEFORE the records land, which is why a second flag exists');
  const css = fs.readFileSync(path.join(REPO, CSS_REL), 'utf8');
  ok(/\.ds-skel-line/.test(css), 'the skeleton is styled');
  ok(/prefers-reduced-motion/.test(css.slice(css.indexOf('.ds-skel-line'))),
     'and it drops the animation under reduced motion, keeping the message');
}

// CLCPA-221: one render per dictionary entry, so the five can be read side by
// side the way an operator sees them.
['layers', 'indicators', 'shapes', 'coned', 'territory'].forEach(function (id) {
  after.api.dict(id, id === 'layers' ? 'layers' : id);
  fs.writeFileSync(path.join(OUT, 'after-dict-' + id + '.html'),
    shell('AFTER: Data Sources, ' + id + ' entry', after.api.page()));
});
after.api.undict();
