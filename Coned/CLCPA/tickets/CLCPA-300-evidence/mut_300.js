/* Mutation controls for suite_300. Temp copy via DAC_APP_OVERRIDE. */
const fs = require('fs');
const os = require('os');
...
const SUITE = path.join(__dirname, 'suite_300.js');
const ORIGINAL = fs.readFileSync(APP, 'utf8');
const TMP = path.join(os.tmpdir(), 'clcpa-300-app.js');
...
const MUTATIONS = [
  {
    label: 'the count goes back to the matched columns',
    from: "    const cols = new Set(d.populated.map(x => x.column)).size;",
    to:   "    const cols = d.matchedColumns.length;",
    expect: ['B1'],
  },
  {
    label: 'the count stops being DISTINCT, so it counts cells not columns',
    from: "    const cols = new Set(d.populated.map(x => x.column)).size;",
    to:   "    const cols = d.populated.length;",
    expect: ['B1', 'B4'],
  },
  {
    label: 'the value count is changed too, which the ruling forbids',
    from: "    const cells = d.populated.length;",
    to:   "    const cells = new Set(d.populated.map(x => x.column)).size;",
    expect: ['B2'],
  },
];
...
log('======================================================================');
log('CLCPA-300 -- mutation controls');
...
if (base.failed !== 0) {
  ...
  fs.writeFileSync(__dirname + '/mut-300-output.txt', lines.join('\n') + '\n');
  process.exit(1);
}
