...
const DIR = _dacRepo() + '/Coned/CLCPA/tickets/CLCPA-242-evidence';
/* THE MUTATION TARGET IS THE PINNED BUILD, not the working tree.
 * suite_242_r2 reads 2361a6a unless DAC_APP_OVERRIDE says
...
const APP = path.join(os.tmpdir(), 'clcpa-242_r2-app-' + NEW_COMMIT + '.js');
...
M.forEach((m) => {
  ...
  let out = '';
  try { out = execFileSync('node', ['suite_242_r2.js'],
...
let cleanOk = true;
try { execFileSync('node', ['suite_242_r2.js'],
    { cwd: DIR, encoding: 'utf8', env: Object.assign({}, process.env, { DAC_APP_OVERRIDE: APP }) }); }
catch (e) {
  cleanOk = false;
