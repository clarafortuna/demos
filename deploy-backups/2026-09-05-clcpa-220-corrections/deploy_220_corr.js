...
const BACKUP_SLUG = '2026-09-05-clcpa-220-corrections';
...
log('org: ' + ORG);
log('slice: CLCPA-220 corrections (row anatomy, flat lists, retire scope)');
...
(async () => {
  ...
  const manifest = { org: ORG, capturedAt: new Date().toISOString(),
    note: 'Pre-deploy snapshot of the live cr2bf_dactest web resources. Restore by ' +
          ...
          'then PublishXml.',
    slice: 'CLCPA-220 corrections', sourceCommit: HEAD, resources: [] };
  const liveSha = {};
  for (const r of RES) {
