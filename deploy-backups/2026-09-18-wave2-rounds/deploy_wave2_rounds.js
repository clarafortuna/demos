 ...
 * ======================================================================= */
const DEPLOY = {
  slug: '2026-09-18-wave2-rounds',
  what: 'the wave-2 rounds from the hosted pass',
  tickets: ['CLCPA-274', 'CLCPA-276', 'CLCPA-277', 'CLCPA-278'],
  /* what this deploy PUSHES, derived from the tree and checked below */
  expectApp: '3b20bdbd6d',
  expectCss: '2b9651445c',
  /* what the ORG is expected to be serving. Read from the org at snapshot
   * time; never derived from git. THE STYLESHEET DOES NOT MOVE in this wave,
   * so the previous and expected stylesheet ids are deliberately the same and
   * that half of the gate proves styles.css UNCHANGED rather than changed. */
  prevApp: 'b0c8c99ebf',
  prevCss: '2b9651445c',
  /* every commit this wave ships, each checked by name as an ancestor */
  commits: [
    { sha: 'ec6d208', what: 'CLCPA-274 round 2: the template carries its second header row' },
    { sha: '062d3f9', what: 'CLCPA-276 round 2: the notice area is repainted with the grid' },
    { sha: '590703c', what: 'CLCPA-277 round 2: the staged year advisory is reachable' },
    { sha: 'd548acc', what: 'CLCPA-278 round 2: the pre-edit row is captured before the first write' },
    { sha: 'e306054', what: 'harness: the four round-2 mutation runners, and what the sweep caught' },
  ],
  /* THE COMMIT THE OWNER ACTED ON. The ids were derived from this HEAD and
   * declared before any write, so the gate binds them to the exact tree. */
  headMustBe: 'b52f604',
  acceptance: [
    'CLCPA-274  THE TEMPLATE CARRIES ITS SECOND HEADER ROW.',
    '   Download F6s template. Row 2 must now read the four sub-labels --',
    '   Non- Excludable / Excludable under NON-NETWORK, and again under',
    '   NETWORK -- instead of being blank, so each of the four value columns',
    '   can be told apart. Check A9 and A10 too: they were blank the same way,',
    '   and A10 was worse, stamping (calculated) into its sub-header. No',
    '   header row may carry that marker. D1 must be unchanged: its first row',
    '   is genuine data and must still be fillable.',
    'CLCPA-276  A NOTICE LIVES EXACTLY AS LONG AS THE DRAFT IT DESCRIBES.',
    '   Type a total that does not add up so the amber advisory appears. Press',
    '   Reset and confirm: the figures restore AND the advisory goes. Repeat',
    '   and switch YEAR instead: it must go, not follow you. Repeat and SAVE:',
    '   it must go. Then the other half, which matters just as much: with a',
    '   draft that still does not add up, add a row -- the advisory must STAY,',
    '   because it is recomputed from the draft rather than cleared.',
    'CLCPA-277  THE STAGED CARD NAMES THE WRONG YEAR.',
    '   Open Add Data, stage a file whose name carries a different year from',
    '   the destination. The staged card must warn, in the same voice as the',
    '   wrong-table twin beside it, and the Add Year button must stay live --',
    '   advisory only. Then RETYPE the year to match the file: the warning must',
    '   disappear as you type, and reappear if you type a third year, WITHOUT',
    '   the year box losing focus. A filename with no year, or two, says',
    '   nothing. The post-load red box must behave exactly as it did.',
    'CLCPA-278  A CONSISTENT TOTAL FOLLOWS THE EDIT.',
    '   In H1, find a row whose Grand Total EQUALS its two parts. Edit one part',
    '   and tab away: the total must move with it, and no advisory is raised.',
    '   Type the number digit by digit -- that is the case round 1 missed.',
    '   Then take a row whose total already DISAGREES and edit a part: the',
    '   total must be left exactly as filed, with the advisory naming it.',
  ],
  disclosed: [
    'A TWO-LEVEL TABLE CANNOT ROUND-TRIP THROUGH A SINGLE-ROW CSV HEADER.',
    '   Measured on this build: F6 2025 import rejections 1 -> 0, A9 2025',
    '   1 -> 3, A10 2025 unchanged. A9s headings genuinely span two rows, so',
    '   neither row alone is a unique heading set and the new rejections read',
    '   two columns with the same heading. The previous build rejected one',
    '   too; emitting the sub-header makes the ambiguity explicit rather than',
    '   hiding it behind a blank row. Needs its own ticket.',
    'THE REMOVE-YEAR CONTROL asks the PUBLISHED payload whether a year holds',
    '   data, not the store. That is why a year holding saves can still offer',
    '   Remove. Read from code, not yet filed.',
    'PRE-EXISTING, not fixed here: on a fresh A8 the CLCPA-240 bootstrap would',
    '   compute 283,852 into the grand total whose real figure is 336,599.',
    '   Unreachable while the row holds numbers.',
...
const TICKET_SYMBOLS = {
  'CLCPA-274': {
    '  function ingestHeaderRowCount(table, rowCount) {': 1,
    /* the predicate is "a number, and at least 2" -- never "has the key", which
     * would claim D1, whose header_levels is 0 and whose first row is data */
    "    if (typeof lv !== 'number' || lv < 2) return 0;": 1,
    /* ONE derivation, BOTH readers: the template writer and the editor */
    '    const templateHeaderRows = ingestHeaderRowCount(table, src.rows.length);': 1,
    '    const headerRowCount = ingestHeaderRowCount(table, i.draft.length);': 1,
    '      if (idx < templateHeaderRows) {': 1,
    /* and the row is emitted as HEADER, so no marker can reach it */
    '          style: XLSX_STYLE_HEADER,': 1,
    /* the editor's retired inline copy must be gone */
    'return Math.min(lv - 1, i.draft.length);': 0,
  },
  'CLCPA-276': {
    /* THE CALL COUNT IS THE DIFFERENTIAL. refreshIngestNotices() is called
     * from the blur handler and, since this round, from the editor repaint --
     * so 2 is the fix and 1 is the build that shipped the defect. Measured on
     * both: b52f604 has 2, the deployed build has 1.
     *
     * A single line on purpose. This half of 3b counts the RAW git blob, which
     * is LF, while the working tree is CRLF -- so a multi-line anchor written
     * with either ending matches nothing and reports "0 of 1". The stylesheet
     * half normalises first; this one never had to until now. */
    '    refreshIngestNotices();': 2,
    '  function rerenderIngestEditor() {': 1,
  },
  'CLCPA-277': {
    /* the destination year is part of the target, which is what made the
     * staged advisory reachable at all */
    '      year: drawYear(),': 1,
    /* and it FOLLOWS the box in place, without redrawing the dialog */
    '    const syncStagedYearWarn = () => {': 1,
    '          syncStagedYearWarn();': 1,
    "      const note = staged ? importYearNotice(staged.name, drawYear()) : null;": 1,
  },
  'CLCPA-278': {
    /* the snapshot is scoped to one wiring and keyed by row */
    '    let preEdit = null;': 1,
    '        if (!preEdit || preEdit.r !== r) {': 1,
    /* read at blur from the INPUT handler's capture, then released */
    '        const beforeRow = (preEdit && preEdit.r === r) ? preEdit.row': 1,
    '        preEdit = null;': 1,
  },
};
