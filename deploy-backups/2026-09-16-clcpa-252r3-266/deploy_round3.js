 ...
 * ======================================================================= */
const DEPLOY = {
  slug: '2026-09-16-clcpa-252r3-266',
  what: 'the round-3 wave',
  tickets: ['CLCPA-252 round 3', 'CLCPA-266'],
  /* what this deploy PUSHES, derived from the tree and checked below */
  expectApp: '804f35afc7',
  expectCss: '04471d05bf',
  /* what the ORG is expected to be serving. Read from the org at snapshot
   * time; never derived from git. */
  prevApp: 'b1108e5fcf',
  prevCss: '25ca2005dc',
  /* every commit this wave ships, each checked by name as an ancestor */
  commits: [
    { sha: '201d566', what: 'CLCPA-252 round 3: no caption carries a year' },
    { sha: 'c732504', what: 'CLCPA-266: the notice boxes' },
    { sha: '0b5c385', what: 'CLCPA-266: main brought into the branch' },
    { sha: '07a9eeb', what: 'CLCPA-266: the merge collateral' },
  ],
  /* THE COMMIT THE OWNER ACTED ON. The approved-id gate is only meaningful
   * when the id is bound to something independent of this file, and for this
   * deploy the ids were derived rather than named in advance -- so they are
   * bound to the merge commit the owner produced by merging PR #244. If HEAD
   * is not that commit, the ids describe a tree nobody approved. */
  headMustBe: '5478ed6',
  acceptance: [
    'CLCPA-252 r3  NO CAPTION CARRIES A YEAR, any table, any year, any surface.',
    '   Open 2023, 2025 and a fresh year: Table C1 reads "Table C1. Summary of',
    '   Con Edison Demand Response Programs" on ALL of them. 51 of 52 derive;',
    '   D2 alone shows its short title. No caption cites a PDF page. Check the',
    '   compare panel and the Report Data editor header too -- same strip.',
    '   A1 on 2023 stored "2023Incentive" with no space and must still read',
    '   "Table A1. Incentive Dollars Spent (Total and in DACs)".',
    'CLCPA-266  EVERY POST-LOAD NOTICE IS A BOX.',
    '   Import a file that raises both advisories. Three boxes, never touching:',
    '   blue "Imported into the draft", AMBER "Read as a fraction" with its',
    '   per-cell lines inside it, RED "Check the table this file was for".',
    '   Same order as before, same words, nothing rejected. In the Add Data',
    '   modal the staged warning is a RED box matching its post-load twin.',
  ],
  disclosed: [
    'C2/2024 stores 766.94, matching neither a sum nor a weighted mean.',
    ...
    'payload.json still stores C5 2025 as "...Summary4"; Dataverse is corrected.',
  ],
};
...
const TICKET_SYMBOLS = {
  'CLCPA-252 round 3': {
    'function stripCaptionYear(s) {': 1,
    'const derived = deriveTableCaption(t, year);': 1,
    'if (stored) return stripCaptionYear(stored);': 1,
    'if (derived) return stripCaptionYear(derived);': 1,
    /* the digit-boundary form: a \b rule left two glued titles carrying their
     * year, and pinning this is what stops that regression returning */
    'out = out.replace(/(\\d?)((?:19|20)\\d{2})(\\d?)/g,': 1,
    "(m, before, y, after) => (before || after) ? m : '');": 1,
    /* the parenthesised-year and preposition tidies */
    "out = out.replace(/\\s*\\(\\s*(?:19|20)\\d{2}\\s*\\)/g, '');": 1,
    'in|for|of|during|through': 1,
    /* and the word-boundary form must be GONE from the strip */
    "out = out.replace(/\\b(?:19|20)\\d{2}\\b/g, '');": 0,
  },
  'CLCPA-266': {
    '<div class="ingest-import-notice is-warn">': 1,
    '<div class="ingest-import-notice is-alert">': 1,
    /* the bare class must no longer be emitted anywhere */
    "'<div class=\"ingest-import-notice\">'": 0,
  },
};

/* the stylesheet half: styles.css moves in this wave */
const CSS_SYMBOLS = {
  '.ingest-import-result,\r\n.ingest-import-notice {': 1,
  '.ingest-import-notice.is-warn { border-left-color: var(--amber); }': 1,
  '.ingest-import-notice.is-alert { border-left-color: var(--red); }': 1,
  '.ingest-import-result > :last-child,\r\n.ingest-import-notice > :last-child { margin-bottom: 0; }': 1,
  'border-left-color: var(--red); border-radius: 6px;': 1,
  /* THE OLD STAGED-WARN BODY must be gone -- that exact pair of lines, not the
   * token on its own. var(--warn-fg, #8a5a00) legitimately SURVIVES once, in
   * .ingest-foot-warn, which this wave does not touch; pinning the bare token
   * at 0 read "1 of 0" and was my fingerprint being wrong, not the build. */
  'margin: 8px 0 0;\r\n  color: var(--warn-fg, #8a5a00);': 0,
  'color: var(--warn-fg, #8a5a00);': 1,
};
