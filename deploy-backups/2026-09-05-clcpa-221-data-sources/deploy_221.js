...
const BACKUP_SLUG = '2026-09-05-clcpa-221-data-sources';
...
log('org: ' + ORG);
log('slice: CLCPA-221 (Data Sources dictionary, session boxes, skeletons)');
...
(async () => {
  ...
  const manifest = { org: ORG, capturedAt: new Date().toISOString(),
    note: 'Pre-deploy snapshot of the live cr2bf_dactest web resources. Restore by ' +
          ...
          'then PublishXml.',
    slice: 'CLCPA-221', sourceCommit: HEAD, resources: [] };
  const liveSha = {};
  for (const r of RES) {
