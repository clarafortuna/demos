 ...
 * ======================================================================= */
const DEPLOY = {
  slug: '2026-09-18-review-followup-wave',
  what: 'the review follow-up wave',
  tickets: ['CLCPA-270', 'CLCPA-274', 'CLCPA-278', 'CLCPA-269',
            'CLCPA-276', 'CLCPA-277', 'CLCPA-275'],
  /* what this deploy PUSHES, derived from the tree and checked below */
  expectApp: 'b0c8c99ebf',
  expectCss: '2b9651445c',
  /* what the ORG is expected to be serving. Read from the org at snapshot
   * time; never derived from git. BOTH halves move in this wave: the
   * stylesheet carries the notice-stack spacing. */
  prevApp: 'b09515e7d8',
  prevCss: '04471d05bf',
  /* every commit this wave ships, each checked by name as an ancestor */
  commits: [
    { sha: '83fd9c1', what: 'CLCPA-270: one locking model, keyed on the row role' },
    { sha: '4de6a8c', what: 'CLCPA-274: the template marks derivable total columns' },
    { sha: 'b902b0b', what: 'CLCPA-274 follow-up: the marker must not reach a header row' },
    { sha: 'ed44cae', what: 'CLCPA-278: the engine figure recomputes, the filed one is kept' },
    { sha: '5d584c5', what: 'CLCPA-269 round 2: the confirm-save count is the preparer cells' },
    { sha: '22c96cc', what: 'CLCPA-276: a notice lives as long as the draft it describes' },
    { sha: '7260063', what: 'CLCPA-277: the wrong year in a filename is named' },
    { sha: '0c25b60', what: 'CLCPA-275: the notice stack is separated from the card below' },
    { sha: '35e15ec', what: 'harness: re-arm and re-attribute the suites this package moved' },
  ],
  ...
  headMustBe: 'f1853ee',
  acceptance: [
    'CLCPA-270  PROTECTION COMES FROM THE LABEL, AND THE FIGURE FROM WHOEVER',
    '   CAN SUPPLY IT. Open A3 2023 in Report Data. Its Total row must have NO',
    '   editable label and NO delete control, and its 2,354,317 must still be',
    '   TYPEABLE -- that is the A8 ruling: the engine cannot derive that figure,',
    '   so it stays the preparers. Same shape on A8 2025 "Total CES Programs',
    '   Installations" (336,599). Then open B1: its Total row must be locked in',
    '   FULL, values included, because the engine does derive it. Every total',
    '   row on every table must still READ as a total row, shaded, whether or',
    '   not its cells are typeable.',
    'CLCPA-274  THE TEMPLATE SAYS WHICH TOTAL COLUMNS IT COMPUTES.',
    '   Download H1s template. Its Grand Total column must read (calculated) on',
    '   the body rows, not sit blank inviting a figure. Then TYPE A NUMBER over',
    '   one and import: it must be ACCEPTED and reconciled, never discarded --',
    '   the marker is guidance, not a rejection. Open F6s template, which has a',
    '   second header row: that header row must NOT carry the marker.',
    'CLCPA-278  A TOTAL THAT AGREES WITH ITS PARTS FOLLOWS AN EDIT.',
    '   In H1, edit a component cell of a row whose Grand Total currently EQUALS',
    '   its parts: the total must move with it. Then edit a component of a row',
    '   whose total already DISAGREES: that total must be KEPT untouched and the',
    '   advisory raised instead. Nothing stored may move until you save.',
    'CLCPA-269  THE SAVE COUNT IS WHAT THE PREPARER BROUGHT.',
    '   Import a file into an EMPTY year and open the Save dialog. The count',
    '   must be the cells you actually filed -- not the label column, not the',
    '   rows the engine computes. On the shape this was filed against it read',
    '   20 and must now read 12.',
    'CLCPA-276  A NOTICE LIVES EXACTLY AS LONG AS THE DRAFT IT DESCRIBES.',
    '   Raise any import notice, then press Reset: it must go. Raise one and',
    '   SAVE: it must go. Raise one, switch table or year and come back: it must',
    '   not be describing a draft that no longer exists.',
    'CLCPA-277  A FILE NAMED FOR THE WRONG YEAR IS NAMED.',
    '   Rename a template to carry a different year and import it into another.',
    '   A RED box must say so, on the staged dialog AND after loading, and the',
    '   import must still go through -- it is advisory only. A filename with no',
    '   year, or two, must say nothing.',
    'CLCPA-275  THE NOTICE STACK CLEARS THE CARD BELOW IT.',
    '   With one or more notices showing at the bottom of the panel, the gap',
    '   below the last one must match the spacing stacked cards already use',
    '   elsewhere. This one is your eye: no harness here can see a rendered gap.',
  ],
  disclosed: [
    'CLCPA-270 locks the LABEL and the delete control on 83 flat-table total',
    '   rows that were open before, across many tables. The values on those rows',
    '   are unaffected. FIVE rows keep typeable values under the A8 ruling:',
    '   A3 2023 r28, A3 2024 r26, A4 2023 r28, A8 2025 r25 and J8 2025 r2.',
    'CLCPA-270 gives back part of the year-to-year stability it bought: a total',
    '   rows VALUE lock now follows whether the engine derives it, which is a',
    '   property of that years data. Its label lock and delete guard do not.',
    'On a year with NO figures yet every total row still locks in full, because',
    '   the engine will compute it once data arrives. Only a stored year whose',
    '   figures fail to reconcile keeps its total typeable.',
    'PRE-EXISTING, not fixed here: on a fresh A8 the CLCPA-240 bootstrap would',
    '   compute 283,852 into the grand total whose real figure is 336,599. It',
    '   is unreachable while the row holds numbers. Needs its own ticket.',
    'payload.json stores 19 KPI entries that disagree with the derive engine,',
    '   and A5 2025 stores 27,833 where its parts give 27,834. Pre-existing,',
    '   pinned, under the payload freeze, and untouched by this wave.',
    'The stylesheet moves in this wave. Both halves of the pair change.',
  ],
};
...
const TICKET_SYMBOLS = {
  'CLCPA-270': {
    'function ingestRowRole(label, tableId, isHeaderRow) {': 1,
    'function ingestRoleOpen(role, engineDerivesValues) {': 1,
    'function isTotalRoleLabel(v, tableId) {': 1,
    'function isComputedShareLabel(v) {': 1,
    'const INGEST_ROLE_OPEN = {': 1,
    /* the renderer asks the accessor, and asks it with the DERIVE ENGINE's own
     * flag for this row -- that pair is the A8 ruling */
    '      const roleOpen = ingestRoleOpen(rowRole, !!editorTotalFlags[rowIdx]);': 1,
    "    if (role !== 'total' || engineDerivesValues) return base;": 1,
    /* all three protections read off what the accessor returned */
    '      const lockTotalRow = !roleOpen.label && !isHeaderRow;': 1,
    '      const isTotal = !roleOpen.values && !isHeaderRow;': 1,
    "        <td class=\"ingest-td-actions\">${!roleOpen.deletable ? ''": 1,
    /* and STYLING stays on the role, so a typeable total still reads as one */
    "      const isTotalRole = rowRole === 'total' || rowRole === 'computed';": 1,
    /* the retired arithmetic-gated label lock, and the retired direct read of
     * the role table, must both be GONE */
    'lockTotalRow = isTotal && isHierFamily': 0,
    'INGEST_ROLE_OPEN[rowRole]': 0,
  },
  'CLCPA-274': {
    '      marksInTemplate: (r, c) => {': 1,
    '        if (computed.marksInTemplate(idx, c)) {': 1,
    '    detectSumColumns(schema, rows, tableId).forEach((s) => { sumRel[s.column] = s; });': 1,
    /* the header-row guard the sweep forced, and the shape it turns on */
    '        const partHasText = rel.parts.some((pc) => {': 1,
    '        return !partHasText;': 1,
    /* THE SEPARATION THIS TICKET EXISTS FOR: the IMPORT still consults `any`,
     * never the template marker, or filed figures start being discarded */
    '        if (computed.any(t.rowIdx, cIdx)) {': 1,
  },
  'CLCPA-278': {
    'function rowSumIsConsistent(row, rel) {': 1,
    'function recomputeDerivableSums(rows, headerRow, tableId, rowIndex, beforeRow, editedCol) {': 1,
    /* the declaration plus its single call site */
    'recomputeDerivableSums(': 2,
  },
  'CLCPA-269': {
    '      const keyCols = Math.max(1, ingestKeyColCount(i.tableId));': 1,
    '          if (c < keyCols) continue;': 1,
    '          if (computed.any(r, c)) continue;': 1,
    '      const computed = ingestComputed(a, i.tableId, i.schema);': 1,
  },
  'CLCPA-276': {
    'function clearIngestNotices(i) {': 1,
    /* the declaration plus the three exits: Reset, a successful save, and the
     * draft load */
    'clearIngestNotices(': 4,
  },
  'CLCPA-277': {
    'function declaredYearFromFilename(name) {': 1,
    'function importYearNotice(fileName, destYear) {': 1,
    /* the declaration plus both surfaces, staged and post-load */
    'importYearNotice(': 3,
  },
};

/* the stylesheet half: styles.css moves in this wave */
/* THE STYLESHEET MOVES IN THIS WAVE: CLCPA-275 is stylesheet-only, so this
 * half is the only fingerprint that ticket has. The two notice-component
 * rules are carried alongside it because the new spacing rule selects the
 * same components -- if a regression removed them, the spacing would apply
 * to elements that no longer exist and the app half would still pass. */
const CSS_SYMBOLS = {
  'CLCPA-275': {
    '.ingest-import-result:last-child,': 1,
    '.ingest-import-notice:last-child { margin-bottom: 16px; }': 1,
    /* the components the new rule SELECTS, carried in the same group: if a
     * regression removed them the spacing would apply to elements that no
     * longer exist, and the app half would still pass green */
...
/* --- 0. this script checks ITSELF, including its own prose ------------- */
{
  ...
  if (!cssSymbolCount()) die('3b has no STYLESHEET symbols to count.');
