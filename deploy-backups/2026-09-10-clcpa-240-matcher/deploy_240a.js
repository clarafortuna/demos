...
const BACKUP_SLUG = '2026-09-10-clcpa-240-matcher';

/* What Emely approved, by name, in advance. */
const EXPECT_APP_ID = '69d8b300a7';
...
const EXPECT_PREV_BUILD = 'b201bb3fcd';
...
const TICKET_SYMBOLS = {
  ...
  'const HIERARCHICAL_TOTALS = { A5: true, A6: true, A7: true, A8: true };': 1,
  ...
  'isGroupHeader && computed.any(idx, c)': 1,
};
...
log('org: ' + ORG);
log('slice: CLCPA-240 first half (hierarchical / composite-key matcher)');
...
try {
  execSync('git merge-base --is-ancestor f0483a0 HEAD', { cwd: REPO });
  log('              and f0483a0, the ticket commit, is an ancestor of HEAD.');
} catch (e) { die('HEAD does not contain f0483a0, the CLCPA-240 commit.'); }
...
(async () => {
  ...
  const manifest = { org: ORG, capturedAt: new Date().toISOString(),
    note: 'Pre-deploy snapshot of the live cr2bf_dactest web resources. Restore by ' +
          ...
          'then PublishXml.',
    slice: 'clcpa-240-matcher', sourceCommit: HEAD, resources: [] };
  const live = {};
  for (const r of RES) {
  ...
  flushLog(null);
  {
    ...
    const msg = 'deploy-backups: pre-deploy snapshot for the CLCPA-240 matcher deploy\n\n' +
  ...
  log('');
  log('THE ACCEPTANCE PASS for CLCPA-240 first half:');
  log('  1. Report Data -> A.5, download the template for 2025.');
  log('  2. Type values into it, save as CSV, import into 2099.');
  log('  3. All 50 rows land, including the 9 group headers and 10 totals.');
  log('  4. The ten group and grand totals COMPUTE, they do not sit blank.');
  log('  5. Save, then reopen: the report reads correctly.');
  log('  6. A.3: the same, and Program Name stays text in every row.');
  log('  7. Re-import the same file into the SAME year: it MATCHES, it does not');
  log('     append duplicates. A5 stays 50 rows, not 60.');
  log('');
  log('Expect 2099 A5 to compute a grand total of 27,834 where 2025 shows the');
  log('stored 27,833. That is the payload discrepancy disclosed in PR #220, it is');
  log('correct-by-route, and it is not a defect in this build.');
  flushLog('=== log written by deploy_240a.js ===');
})().catch(e => {
