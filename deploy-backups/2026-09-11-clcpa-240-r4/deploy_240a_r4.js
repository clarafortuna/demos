...
const BACKUP_SLUG = '2026-09-11-clcpa-240-r4';

/* What Emely approved, by name, in advance. */
const EXPECT_APP_ID = 'b8c5b6801a';
...
const EXPECT_PREV_BUILD = '92b1a65b17';
...
const TICKET_SYMBOLS = {
  ...
  /* the lockTotalRow line is asserted once, in the round-4 block below. It was
   * listed here as well; a repeated key in an object literal simply collapses,
   * so the count was 31 either way and nothing was mis-reported -- the second
   * entry is gone for tidiness, not as a correction. */
  ...
  /* --- round 4 ---------------------------------------------------------- */
  'if (!isHierarchicalTotalLabel((rows[i] || [])[0])) out[i] = false;': 1,
  /* the veto must be the ONLY place a flag is cleared, and it must sit inside
   * the declared-family branch -- the count is what proves the first, and
   * suite_240a_r4's S2b proves the second by brace matching */
  'out[i] = false': 1,
  /* round 3's form of this line gained the label condition in round 4, so the
   * fingerprint tracks the new one. A copied fingerprint's quoted lines are as
   * perishable as the code: this exact assertion failed the round-3 deploy on
   * its first attempt, which is the gate working. */
...
function flushLog(tail) {
  ...
  try {
    if (ARCHIVED || fs.existsSync(BACKDIR + '/manifest.json')) {
      ...
      fs.writeFileSync(path.join(__dirname, 'deploy_240a_r4_abort.log'),
        lines.join('\n') + '\n');
    }
...
log('org: ' + ORG);
log('slice: CLCPA-240 round 4 (the label has a veto in four tables)');
...
try {
  execSync('git merge-base --is-ancestor d9d7625 HEAD', { cwd: REPO });
  log('              and d9d7625, the round-4 commit, is an ancestor of HEAD.');
} catch (e) { die('HEAD does not contain d9d7625, the CLCPA-240 round-4 commit.'); }
...
(async () => {
  ...
  const manifest = { org: ORG, capturedAt: new Date().toISOString(),
    note: 'Pre-deploy snapshot of the live cr2bf_dactest web resources. Restore by ' +
          ...
          'then PublishXml.',
    slice: 'clcpa-240-r4', sourceCommit: HEAD, resources: [] };
  const live = {};
  for (const r of RES) {
  ...
  flushLog(null);
  {
    ...
    const msg = 'deploy-backups: pre-deploy snapshot for the CLCPA-240 round-4 deploy\n\n' +
  ...
  log('');
  log('THE ACCEPTANCE PASS for CLCPA-240 round 4:');
  log('  1. THE SCREEN THAT FAILED. A.5, a fresh year, import, and type 999');
  log('     into EVERY data row. Each group total must equal that group s own');
  log('     rows, and the grand total the whole table. You saw 14,985 where');
  log('     the group holds 1,998.');
  log('  2. No DATA row renders grey. Building Shell and HVAC are ordinary');
  log('     editable rows again, with their x.');
  log('  3. Repeat with VARIED figures: still correct, as it already was.');
  log('  4. Round 3 scope unchanged: nine headers locked as bands on the');
  log('     unsaved import, hand-typed rows editable.');
  log('  5. A.1, a FLAT table: its Total row still has an editable label and');
  log('     an x. CLCPA-205 item 2 stays pending.');
  log('');
  log('STILL DISCLOSED, not defects in this build:');
  log('  - A8:2025 row 25 stays EDITABLE: it holds 336,599 where A8 own rows');
  log('    sum to 283,852, so arithmetic cannot confirm it.');
  log('  - 2099 A5 computes a grand total of 27,834 where 2025 shows the');
  log('    stored 27,833. Correct-by-route, PR #220.');
  log('  - A draft where NOTHING has a value locks no headers.');
  log('  - The render still calls recomputeTotals before its flags, so two');
  log('    flags are still lost across a render, down from seven. REDUCED,');
  log('    not fixed; it belongs to the sparse-inference ticket.');
  log('  - The general confirms() correction for all 52 tables is NOT in this');
  log('    build. The veto is scoped to A5, A6, A7 and A8.');
  flushLog('=== log written by deploy_240a_r4.js ===');
})().catch(e => {
