...
const BACKUP_SLUG = '2026-09-05-clcpa-220-pr2-rollback';
...
log('org: ' + ORG);
log('slice: CLCPA-220 PR 2 (version lists, rollback, activation confirm, rename)');
...
(async () => {
  ...
  const manifest = { org: ORG, capturedAt: new Date().toISOString(),
    note: 'Pre-deploy snapshot of the live cr2bf_dactest web resources. Restore by ' +
          ...
          'then PublishXml.',
    slice: 'CLCPA-220 PR 2', sourceCommit: HEAD, resources: [] };
  const liveSha = {};
  for (const r of RES) {
