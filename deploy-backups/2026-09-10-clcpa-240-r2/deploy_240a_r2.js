...
log('org: ' + ORG);
log('slice: CLCPA-240 round 2 (group headers and totals are structure)');
...
try {
  ...
  log('              and 68567b8, the round-2 tip, is an ancestor of HEAD.');
} catch (e) { die('HEAD does not contain 68567b8, the CLCPA-240 round-2 commit.'); }
...
(async () => {
  ...
  const manifest = { org: ORG, capturedAt: new Date().toISOString(),
    note: 'Pre-deploy snapshot of the live cr2bf_dactest web resources. Restore by ' +
          ...
          'then PublishXml.',
    slice: 'clcpa-240-r2', sourceCommit: HEAD, resources: [] };
  const live = {};
  for (const r of RES) {
  ...
  flushLog(null);
  {
    ...
    const msg = 'deploy-backups: pre-deploy snapshot for the CLCPA-240 round-2 deploy\n\n' +
  ...
  log('');
  log('THE ACCEPTANCE PASS for CLCPA-240 round 2:');
  log('  1. Report Data -> A.5. The 9 group header rows render as BANDS: no');
  log('     input to type into, and no x to delete them.');
  log('  2. The 10 total rows the same, LABEL included, and no x.');
  log('  3. The 31 ordinary data rows KEEP their inputs and their x. The lock');
  log('     must not have spread.');
  log('  4. + Add Row, type a program name, then + Add Row again: the row you');
  log('     were filling in stays editable. It must not lock under you.');
  log('  5. Download A.5s template: heading rows read (no value) across their');
  log('     columns, total rows still read (calculated), and the Instructions');
  log('     sheet explains both.');
  log('  6. Import it into a fresh year: still 50 rows, totals still compute.');
  log('     Re-import into the SAME year: still 50, nothing appended.');
  log('  7. A.1, a FLAT table: its Total row still has an editable label and');
  log('     an x. CLCPA-205 item 2 is deliberately untouched.');
  log('');
  log('TWO THINGS THAT ARE NOT DEFECTS IN THIS BUILD, both disclosed:');
  ...
  log('    arithmetic cannot confirm it and the lock does not reach it. Ratified');
  log('    to stay unlocked rather than freeze a number nothing can recompute.');
  log('  - 2099 A5 computes a grand total of 27,834 where 2025 shows the stored');
  log('    27,833. Correct-by-route, disclosed in PR #220.');
  flushLog('=== log written by deploy_240a_r2.js ===');
})().catch(e => {
