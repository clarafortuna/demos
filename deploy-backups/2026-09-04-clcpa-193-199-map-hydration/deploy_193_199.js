...
const BACKUP_SLUG = '2026-09-04-clcpa-193-199-map-hydration';
...
log('org: ' + ORG);
log('slice: CLCPA-193 (metadata-first saved-layer hydration) + CLCPA-199 (pointer hover)');
...
(async () => {
  ...
  const manifest = { org: ORG, capturedAt: new Date().toISOString(),
    note: 'Pre-deploy snapshot of the live cr2bf_dactest web resources. Restore by ' +
          ...
          'then PublishXml.',
    slice: 'CLCPA-193 + CLCPA-199', sourceCommit: HEAD, resources: [] };
  const liveSha = {};
  for (const r of RES) {
