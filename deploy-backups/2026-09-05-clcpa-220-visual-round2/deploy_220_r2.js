...
const BACKUP_SLUG = '2026-09-05-clcpa-220-visual-round2';
...
log('org: ' + ORG);
log('slice: CLCPA-220 visual round 2 (button-family tabs, pills removed)');
...
(async () => {
  ...
  const manifest = { org: ORG, capturedAt: new Date().toISOString(),
    note: 'Pre-deploy snapshot of the live cr2bf_dactest web resources. Restore by ' +
          ...
          'then PublishXml.',
    slice: 'CLCPA-220 visual round 2', sourceCommit: HEAD, resources: [] };
  const liveSha = {};
  for (const r of RES) {
