...
const BACKUP_SLUG = '2026-09-05-clcpa-220-round4-fixes';
...
log('org: ' + ORG);
log('slice: CLCPA-220 round 4 (tab fill fix, About click fix, title case)');
...
(async () => {
  ...
  const manifest = { org: ORG, capturedAt: new Date().toISOString(),
    note: 'Pre-deploy snapshot of the live cr2bf_dactest web resources. Restore by ' +
          ...
          'then PublishXml.',
    slice: 'CLCPA-220 round 4', sourceCommit: HEAD, resources: [] };
  const liveSha = {};
  for (const r of RES) {
