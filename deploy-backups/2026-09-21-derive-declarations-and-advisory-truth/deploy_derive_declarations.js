 ...
 * ======================================================================= */
const DEPLOY = {
  slug: '2026-09-21-derive-declarations-and-advisory-truth',
  what: 'the derive engine declares what it computes and the advisories tell the truth: column totals on the G board and B2s crossing pair, the workbook marker gated on derivability, percentage rows registered as computed, the reconciliation advisory read from the draft, and the history counting operator changes only',
  tickets: ['CLCPA-311', 'CLCPA-319', 'CLCPA-320', 'CLCPA-308', 'CLCPA-304',
            'CLCPA-307', 'CLCPA-310', 'CLCPA-309', 'CLCPA-305', 'CLCPA-306',
            'CLCPA-302', 'CLCPA-303'],
  /* what this deploy PUSHES, named by the owner in advance and re-derived
   * from the real merged HEAD before any write. */
  expectApp: 'bf2b92af29',
  expectCss: '9039bfc509',
  /* what the ORG is expected to be serving, named by the owner and proven
   * from ORG CONTENT at snapshot time, never derived from git. THE
   * STYLESHEET DOES NOT MOVE this wave: the repo has not touched it since
   * the live build, so the previous and expected ids are deliberately
   * identical and that half of the gate proves styles.css UNCHANGED. */
  prevApp: '16eda8c41e',
  prevCss: '9039bfc509',
  /* every commit this wave ships, each checked by name as an ancestor.
   * Eight pull requests merged in order; six of them carry code. The two
   * that do not are records: a measurement that reproduced a discard and
   * reversed an earlier verdict, and a candidate built, measured and
   * withdrawn before merge. They are listed because they merged, not
   * because they change a byte. */
  commits: [
    { sha: '5a3b11a', what: 'the LMI share is read, not invented' },
    { sha: '2be55b4', what: 'the derive-engine program: column totals, the marker, the D rows' },
    { sha: '4094c0a', what: 'the small B and D board tickets' },
    { sha: '24c1642', what: 'the reconciliation advisory' },
    { sha: '40b74e2', what: 'the history counts operator changes only' },
    { sha: '95a6f01', what: 'the stored-anatomy discard reproduced, records only' },
    { sha: 'a331392', what: 'both total directions declared on B2, the marker amendment, the fraction advisory' },
    { sha: '700f6e9', what: 'the persist-strip follow-up, records only' },
  ],
  headMustBe: '8bf5ea7',
  acceptance: [
    'THE HEADLINE GATE, AND IT IS A NEGATIVE ONE: NOTHING PUBLISHED MOVES.',
    '   Every figure on every section page should read exactly as it does',
    '   today, to the digit. This wave declares what the engine already',
    '   computed; it was measured at 0 moved table-years and 0 moved KPI',
    '   figures. Any changed published number is a regression, not a fix.',
    '',
    'THE G BOARD, ON AN IMPORTED YEAR. Add a scratch year on G1, download',
    '   the template, and fill ONLY the two category rows: 9,999 feet in a',
    '   DAC and 999 not. Leave the Systemwide Total alone. Import and look',
    '   at the section page: the total must read 10,998 and 100.0%, not',
    '   blank. Before this it rendered flagged and EMPTY.',
    '',
    'B2, THE TWO TOTALS CROSSING. Open Report Data, B2, 2025. The Total row',
    '   and the Total Plugs column must read as they do today: 7,482 /',
    '   1,399 / 3 / 8,884.',
    '   NOW TYPE OVER ONE. Put any wrong number into the Total row under',
    '   L2 Plugs. It must be KEPT, and an amber advisory beneath the grid',
    '   must name it. Before this the engine silently overwrote it.',
    '   AND THE OTHER AXIS still behaves: a wrong number typed into a',
    '   category rows Total Plugs is kept and named exactly as before.',
    '',
    'THE WORKBOOK MARKER. Download J8s template. Its Total row must read',
    '   fill-in, NOT (calculated): those dollar figures belong to the',
    '   preparer and the engine cannot derive them. Then download A5s: its',
    '   Total row must still read (calculated), because that one it can.',
    '',
    'THE FRACTION ADVISORY. Open Report Data, D3, and type 22.4% into',
    '   "Percentage of subscribers in DACs". NO advisory may appear saying',
    '   "22.4% read as 0.224": the engine recomputes that cell and the',
    '   number would never survive. Now type 22.4% into a COUNT row',
    '   instead: there the advisory must still appear, because there it is',
    '   true.',
    '',
    'THE SAVE COUNT. Open any table, change ONE cell an operator can type',
    '   into, and press Save Changes. The dialog and the history must agree',
    '   on the number of changes. Before this the history counted engine',
    '   cells the dialog had excluded.',
  ],
  disclosed: [
    'B2 IS NOT IN THE PERSIST STRIP, by ruling. Its total row is now',
    '   derivable, so the stored copy is a copy of something computable. A',
    '   strip change needs its own round-trip proof and its own turn, and',
    '   the declaration reconciles and advises on divergence meanwhile, so',
    '   the copy cannot drift silently. Recorded as a follow-up.',
    'THE IMPORT GUARD STILL USES A VALUE CLASSIFIER. A filed grand total on',
    '   a table whose stored figure happens to reconcile is still refused in',
    '   silence. A fix was built under ruling, measured, and WITHDRAWN before',
    '   merge: read literally it hands 262 of 432 total-row cells to the',
    '   preparer, which would undo the protection against a file overwriting',
    '   a figure the engine derives. The corrected form is an explicit',
    '   registry of totals that belong to the preparer, and it is the next',
    '   round. Nothing in this build regressed; the defect is unchanged.',
    'THE SECOND SURFACE, MEASURED AND UNFIXED: an accepted preparer total',
    '   does not survive the editors recompute. It is not reachable in this',
    '   build because the import refuses those cells first, and it is why',
    '   the next round has to cover both surfaces together.',
    'A3s WORKBOOK MARKER IS YEAR-DEPENDENT AGAIN, as the cost of gating the',
    '   marker on derivability: A3/2023, A3/2024 and A4/2023 lose it. Every',
    '   other moved row is a BLANK template, so no workbook a preparer can',
    '   download today changes.',
    'THE STYLESHEET AND THE HTML DO NOT MOVE in this wave. app.js alone',
    '   changes; the HTML follows it through its ?v= stamp, and the byte',
    '   comparison against the live copy is what decides what is pushed,',
    '   not this note.',
  ],
};
...
const TICKET_SYMBOLS = {
  'CLCPA-311': {
    /* the LMI share is read from its own percentage row rather than divided
     * out of two counts that do not describe it */
    "      const d3LmiPct  = getDRow('D3', ['percentage', 'low-income', 'energy affordability']);": 1,
    '      const dBarMetric = (label, fmtFn, total, dac, prevTotal, prevDac, tableId, isLmi, share) => {': 1,
  },
  'CLCPA-319': {
    /* the column-total rule, built in one place, and the formatter that keeps
     * a derived total reading like a figure in the editor */
    '    const colTotal = (column) =>': 1,
    '  function fmtDerivedCell(v, d, isMoney) {': 1,
    "    if (d.type === 'columnTotal') return formatIngestValue(v, isMoney);": 1,
  },
  'CLCPA-320': {
    /* the workbook marker consults DERIVABILITY, not row role alone */
    '        if (((!!totals[r] || !!totalRole[r]) && engineDerives(r, c) &&': 1,
  },
  'CLCPA-308': {
    /* declared derived ROWS are computed cells to the workbook */
    '    ((tableId && DERIVED_ROWS[tableId]) || []).forEach((d) => { derivedRowSet[d.row] = d; });': 1,
    '        if (!!derivedRowSet[r] && c >= 1 && engineWrites(c)) return true;': 1,
  },
  'CLCPA-304': {
    /* one schema reader on the template path, not a second idea of it */
    '    const schema = getTableSchema(table, yr) || [];': 1,
  },
  'CLCPA-307': {
    /* the operator guide names the controls as they read on screen */
    "        '4. In the dashboard, open Report Data and press Add Data, choose this ' +": 1,
  },
  'CLCPA-310': {
    /* the fraction advisory formats the value it shows */
    '  function unitNoticeValue(raw, landed) {': 1,
    '    const places = Math.min(typed + 2, 12);': 1,
  },
  'CLCPA-309': {
    /* and does not fire at all on a cell the engine recomputes, on either
     * surface */
    '        if (isPercentLiteral(raw) && !pctCols[cIdx] &&': 1,
    '    if (!isPercentLiteral(raw) || pctCols[c] || onDerivedRow) {': 1,
  },
  'CLCPA-305': {
    /* the reconciliation advisory is read from the draft, in one place */
    '      renderReconcileNotice(draftReconcile) +': 1,
  },
  'CLCPA-306': {
    /* and a cell the engine itself wrote is not a figure somebody filed */
    '      if (isTotalRow(n.rowIndex)) return;': 1,
    '      if (!isTotalRow(n.rowIndex)) return true;': 1,
  },
  'CLCPA-302': {
    /* the history counts what the dialog counted */
    '    function diffRows(oldRows, newRows, schema, tableId) {': 1,
  },
  'CLCPA-303': {
    /* both of B2s total directions declared, and one writer for the cell
     * where they cross */
    '      B2: [colTotal(1), colTotal(2), colTotal(3), colTotal(4)],': 1,
    "    if (((tableId && DERIVED_COLS[tableId]) || []).some(d => d.type === 'columnTotal') &&": 1,
  },
};
